...
# オプション
uv run python run_gui.py input_generated/ --port 8080      # ポート変更
uv run python run_gui.py input_generated/ --no-browser     # ブラウザを開かない
```
...
```
[編集] → [保存] → transcript_unexported.json
                        ↓
                   [書き出し]
                        ↓
...
- **書き出し**: 差分のみ処理（変更/追加→書き出し、テキストのみ変更→リネーム、削除→ファイル削除）

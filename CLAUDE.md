...
2. **セグメント編集**: ドラッグ操作または数値入力で時刻調整
3. **テキスト編集**: 文字起こしテキストの修正
4. **ファイル名自動生成**: テキストから自動生成（編集不可）
5. **音声再生**: セグメント単位の再生・ループ再生
6. **自動選択**: 再生カーソル位置のセグメントを自動選択
7. **保存**: `transcript_unexported.json`として保存（編集中の状態）
8. **差分書き出し**: 変更/追加/削除/リネームを差分ベースで処理
9. **全件書き出し**: 強制的に全セグメントを再書き出し
...
- ファイル名は`{index:03d}_{text}.{ext}`形式で自動生成
- セグメントは開始時刻順にソートして表示
